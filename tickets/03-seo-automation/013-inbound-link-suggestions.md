...
- [ ] Inbound suggestions never include the new article itself.
- [ ] An existing article that already links to the new article is excluded.
- [ ] `anchorText` is validated against the **existing** article's text, not the new one's —
      asserted by a test where the anchor appears only in the new article and is therefore dropped.
- [ ] Each suggestion identifies the source article to edit as well as the target URL.
- [ ] Deterministic candidate ordering, as in T-03-011.
- [ ] With a corpus of 1 article, inbound suggestions are empty rather than throwing.
