...
- [ ] Unit tests cover every branch: `meta_title` override; a 46-character title (no suffix);
...
- [ ] Truncation never splits a word and the result is at most 70 characters.
- [ ] Called twice with the same input, it returns the identical string.
- [ ] Every produced title is at least 10 characters (Spec 02 rule `title-length` lower bound).
- [ ] A title consisting only of whitespace in `meta_title` falls through to step 2 rather than
      being used.
